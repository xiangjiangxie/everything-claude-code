...
4. **Task（Explore 代理）**：对于更广泛的探索，使用 `Task` 配合 `subagent_type: "Explore"`
...
3. 每个使用计划中对应的 `SESSION_ID` 进行 `resume`（如缺失则创建新会话）
...
2. 按信任规则加权：后端遵循 Codex，前端遵循 Gemini
3. 执行必要修复
