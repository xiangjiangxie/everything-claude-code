...
- **RAII 无处不在** — 不使用手动 `new`/`delete`
- 使用 `std::unique_ptr` 实现独占所有权
- 仅在真正需要共享所有权时使用 `std::shared_ptr`
- 使用 `std::make_unique` / `std::make_shared` 替代裸 `new`
...
- 使用 **clang-format** — 无风格争论
- 提交前运行 `clang-format -i <file>`

## 参考

参阅技能：`cpp-coding-standards` 了解全面的 C++ 编码标准和指南。
